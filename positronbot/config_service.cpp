...
static void add_server_address(
  pb::config_server_t& server,
  std::string& astr
) {
  pb::config_address_t addr;
...
static void add_server(
  ...
  server.nick = "PositronBot";
  server.user = "positron";
...
pb::config_service::config_service(std::string _path)
: path(_path)
{}

pb::config_service::~config_service() {}

pb::config_t pb::config_service::get_config() {
  pb::config_t cfg;
  cfg.prefix = "!";
