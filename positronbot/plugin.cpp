...
static std::string find_path_to_plugin(std::string name, const std::vector<std::string>& plugin_dirs) {
  const std::vector<std::string> paths {
    "./libpb-" + name + ".so",
    "/usr/share/libpb-" + name + ".so",
    "/usr/lib/libpb-" + name + ".so",
    "/usr/local/share/libpb-" + name + ".so",
    "/usr/local/lib/libpb-" + name + ".so"
  };
  ...
  for (std::string dir : plugin_dirs) {
    std::string path = dir + "/libpb-" + name + ".so";
    if (std::ifstream(path).good())
      return path;
  }
...
namespace pb {
  ...
  plugin::plugin(particledi::dm_ptr _dm, std::string& _name, std::vector<std::pair<std::string, std::string>>& cfg, const std::vector<std::string>& plugin_dirs)
  : dm(_dm)
  , handle(nullptr)
  , name(_name)
  , l("?" + name)
  {
    std::string path = find_path_to_plugin(name, plugin_dirs);
    ...
    plugin** p = (plugin**) dlsym(handle.get(), "pb_plugin");
    if ((error = dlerror()) == nullptr) {
      *p = this;
    }
    
    init_func init_f = (init_func) dlsym(handle.get(), "pb_init");
    if ((error = dlerror()) == nullptr) {
      init_f(cfg);
    }
  ...
  plugin::~plugin() {
    ...
    deinit_func deinit_f = (deinit_func) dlsym(handle.get(), "pb_deinit");
    if ((error = dlerror()) == nullptr) {
      deinit_f();
    }
  }
