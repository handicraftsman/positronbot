...
namespace pb {
  ...
  public:
    ...
    plugin(particledi::dm_ptr dm, std::string& name, std::vector<std::pair<std::string, std::string>>& cfg, const std::vector<std::string>& plugin_dirs);
    ~plugin();
...
#define pb_ptrcast std::reinterpret_pointer_cast
