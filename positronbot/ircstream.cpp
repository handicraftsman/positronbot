...
pb::ircstream::ircstream(pb::ircsocket_base& _socket)
: std::ostream(this)
, socket(_socket)
{}

pb::ircstream::~ircstream() {}

std::streambuf::int_type pb::ircstream::overflow(std::streambuf::int_type c) {
  if (c != EOF) {
...
std::string pb::ircstream::nick(std::string nickname) {
  return "NICK " + nickname + "\r\n";
}

std::string pb::ircstream::auth_pass(std::string password) {
  return "PASS " + password + "\r\n";
}

std::string pb::ircstream::auth_user(std::string username, std::string realname) {
...
std::string pb::ircstream::privmsg(std::string target, std::string message) {
  std::stringstream ss;
...
std::string pb::ircstream::notice(std::string target, std::string message) {
  std::stringstream ss;
...
std::string pb::ircstream::ctcp(std::string target, std::string type, std::string message) {
  format(message);
  return "PRIVMSG " + target + " :\x01" + type + " " + message + "\x01\r\n";
}

std::string pb::ircstream::nctcp(std::string target, std::string type, std::string message) {
  format(message);
  return "NOTICE " + target + " :\x01" + type + " " + message + "\x01\r\n";
}

std::string pb::ircstream::join(std::string channel) {
  return ("JOIN " + channel + "\r\n");
}

std::string pb::ircstream::join(std::string channel, std::string password) {
  return ("JOIN " + channel + " " + password + "\r\n");
}

std::string pb::ircstream::part(std::string channel, std::string reason) {
  return ("PART " + channel + " :" + reason + "\r\n");
}

std::string pb::ircstream::who(std::string channel) {
  return ("WHO " + channel + "\r\n");
}
